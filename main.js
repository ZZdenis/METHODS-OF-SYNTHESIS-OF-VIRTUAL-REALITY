...
function draw() {
    ...
    let xPos = Math.sin(timestamp);
    let zPos = Math.sin(timestamp);
    let translationMatrix = m4.translation(xPos, 0, zPos);
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, translationMatrix);
    sphere.Draw();
    if (panner) {
...
function initGL() {
    ...
    plane.BufferData2(planeVertices());
    plane.BufferData3(planeTextures());
...
function init() {
    ...
    initAudioCtx()
    let canvas;
    stereoCamera = new StereoCamera(conv, eyes, 1, fov, near, 100.0);
...
function LoadTexture() {
    ...
    image.src = "https://raw.githubusercontent.com/ZZdenis/VISUALIZATION-OF-GRAPHICAL-AND-GEOMETRIC-INFORMATION/CGW/abstract_texture.jpg";
    image.onload = () => {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(
            gl.TEXTURE_2D,
...
function readValues() {
    ...
    stereoCamera.mConvergence = parseFloat(conv);
    stereoCamera.mEyeSeparation = parseFloat(eyes);
    stereoCamera.mFOV = parseFloat(fov) * Math.PI / 180.0;
    stereoCamera.mNearClippingDistance = parseFloat(near);
}
